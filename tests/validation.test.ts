...
describe("catalogue validation",()=>{
  ...
  it("returns actionable product field messages",()=>{const result=productCreateSchema.safeParse({slug:"Invalid Slug",nameZh:"A",nameEn:"B",descriptionZh:"",descriptionEn:"",category:"項鏈",collection:"LUMEA",audience:"PEOPLE",material:"",gemstone:"",imageUrl:"https://example.com/item.jpg",featured:false,sku:"A",optionName:"",priceMinor:0,stockOnHand:-1});expect(result.success).toBe(false);if(!result.success){const errors=result.error.flatten().fieldErrors;expect(errors.slug).toContain("只可使用小寫英文字母、數字及連字號。");expect(errors.sku).toContain("SKU 最少需要 3 個字元。");expect(errors.priceMinor).toContain("價格必須為 HK$1 或以上。");}});
