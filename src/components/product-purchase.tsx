...
export function ProductPurchase({ productId, slug, variants, locale = "zh-HK" }: { productId: string; slug: string; variants: Variant[]; locale?: Locale }) {
  ...
  async function add() {
    ...
    setMessage(response.ok ? (en ? "Added to your shopping bag." : "作品已加入購物袋。") : (en ? "Unable to add this piece. Please try again." : data.error));
    setBusy(false);
  }
  async function wish() {
