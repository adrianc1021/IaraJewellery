...
export function SiteHeader() {
  ...
  </div><nav className={`mobile-nav ${open ? "open" : ""}`} aria-hidden={!open}>{links.map(([label, href]) => <Link key={label} href={href} onClick={() => setOpen(false)}>{label}</Link>)}<Link href={session ? "/account" : "/login"} onClick={() => setOpen(false)}>{session ? "會員中心" : "登入／申請會員"}</Link></nav></header>;
}
