...
export function SiteHeader({ locale }: { locale: Locale }) {
  ...
  const links = en ? [["New", "/shop?sort=newest"], ["Jewellery", "/shop"], ["Collections", "/shop?view=collections"], ["Bridal", "/shop?collection=ARIA+BRIDAL"], ["Pets", "/pets"], ["Atelier", "/appointment"]] : [["新品", "/shop?sort=newest"], ["珠寶", "/shop"], ["系列", "/shop?view=collections"], ["婚嫁", "/shop?collection=ARIA+BRIDAL"], ["寵物飾品", "/pets"], ["門市", "/appointment"]];
  ...
  return <header className={`site-header ${scrolled ? "scrolled" : ""} ${pathname === "/" ? "home-header" : ""}`}><div className="header-inner container">
    ...
    <nav className="desktop-nav" aria-label={en ? "Main navigation" : "主要導覽"}>{links.map(([label, href]) => <Link key={label} href={href}>{label}</Link>)}</nav>
