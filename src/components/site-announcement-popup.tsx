...
export function SiteAnnouncementPopup() {
  ...
  return <div className={`site-popup ${visible ? "visible" : ""}`} aria-hidden={!visible}>
    <button className="site-popup-backdrop" aria-label="關閉通告" onClick={dismiss} tabIndex={visible ? 0 : -1} />
    <section className={`site-popup-dialog ${announcement.imageUrl ? "with-image" : ""}`} role="dialog" aria-modal="true" aria-labelledby="site-popup-title">
      {announcement.imageUrl && <div className="site-popup-image" style={{ backgroundImage: `url(${announcement.imageUrl})` }} role="img" aria-label="Iara 通告圖片" />}
      <div className="site-popup-copy"><button className="icon-button site-popup-close" onClick={dismiss} aria-label="關閉通告"><X size={18} /></button><p className="eyebrow">{announcement.eyebrow || "IARA PRIVATE NOTICE"}</p><h2 id="site-popup-title">{announcement.title}</h2><p>{announcement.body}</p>{announcement.ctaLabel && announcement.ctaHref && <Link className="button button-primary" href={announcement.ctaHref} onClick={dismiss}>{announcement.ctaLabel}<ArrowRight size={15} /></Link>}<button className="site-popup-dismiss" onClick={dismiss}>暫時不用</button></div>
    </section>
  </div>;
}
