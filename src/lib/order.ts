...
export async function createPendingOrder(request: Request, raw: unknown, idempotencyKey: string) {
  ...
  if (existing?.response) return JSON.parse(existing.response) as { orderId: string; orderNumber: string; totalMinor: number };
  ...
  return db.$transaction(async (tx) => {
    ...
    if (currentKey?.response) return JSON.parse(currentKey.response) as { orderId: string; orderNumber: string; totalMinor: number };
    ...
    const order = await tx.order.create({ data: { orderNumber, userId: session?.user.id, email: input.email, customerName: input.customerName, phone: input.phone, subtotalMinor, discountMinor, totalMinor: subtotalMinor - discountMinor, deliveryMethod: input.deliveryMethod, shippingAddress: input.shippingAddress, giftMessage: input.giftMessage, promotionCode: input.promotionCode?.toUpperCase(), items: { create: cart.items.map((item) => ({ productId: item.variant.product.id, variantId: item.variant.id, productName: item.variant.product.nameZh, sku: item.variant.sku, optionName: item.variant.optionName, unitPriceMinor: item.variant.priceMinor, quantity: item.quantity, lineTotalMinor: item.variant.priceMinor * item.quantity })) } } });
    ...
    const response = { orderId: order.id, orderNumber: order.orderNumber, totalMinor: order.totalMinor };
