...
export default async function HomePage() {
  ...
  return <main id="main" className="page-shell homepage-layout" style={style}>
    ...
    <section className="category-chapter"><header className="category-chapter-heading reveal-item"><p className="eyebrow">SHOP BY CATEGORY</p><h2>{en ? "Find the piece you are looking for" : "按珠寶類別選購"}</h2><p>{en ? "Browse rings, necklaces, earrings and bracelets." : "從戒指、項鏈、耳環及手鏈之間，直接找到合適作品。"}</p></header><div className="category-strip" aria-label={en ? "Shop by category" : "按分類選購"}>{categories.map((category, index) => <Link key={category.id} href={`/shop?category=${encodeURIComponent(category.nameZh)}`}><span className="category-image"><Image src={categoryImages[category.nameZh] || category.imageUrl || "https://images.unsplash.com/photo-1515562141207-7a88fb7ce338?auto=format&fit=crop&w=900&q=86"} alt={en ? category.nameEn : `${category.nameZh}系列`} fill sizes="(max-width: 680px) 50vw, 25vw" /><i>{String(index + 1).padStart(2, "0")}</i></span><span className="category-label"><small>{category.nameEn.toUpperCase()}</small>{en ? category.nameEn : category.nameZh}<em>{en ? "Explore edit" : "探索系列"} <ArrowRight size={13} /></em></span></Link>)}</div></section>
    ...
    <section className="editorial-band" id="story"><div className="editorial-copy reveal-item"><div className="craft-mark"><span>IA</span><small>ATELIER / 2026</small></div><p className="eyebrow">THE ART OF IARA</p><h2>{en ? <>Craft you can<br /><i>feel</i></> : <>讓工藝，成為<br /><i>可感受的溫度</i></>}</h2><p>{en ? "From stone selection and hand setting to the final polish, every piece is carefully inspected in our Hong Kong atelier." : "從寶石甄選、手工鑲嵌到最後拋光，每一件作品皆在香港工房經過細緻檢查，讓光線、弧度與肌膚恰好相遇。"}</p><ol className="craft-proof"><li><small>01</small><span>{en ? "Stone selection" : "寶石甄選"}</span><em>{en ? "The first light" : "尋找第一道光"}</em></li><li><small>02</small><span>{en ? "Hand setting" : "手工鑲嵌"}</span><em>{en ? "One stone at a time" : "逐石細緻鑲嵌"}</em></li><li><small>03</small><span>{en ? "Polish and inspection" : "拋光與品質檢查"}</span><em>{en ? "A quiet final check" : "最後的安靜檢查"}</em></li></ol><Link className="text-link" href="/journal">{en ? "See how each piece is made" : "了解每件作品如何製作"} <ArrowRight size={13} /></Link></div><div className="editorial-image"><Image src="https://images.unsplash.com/photo-1584302179602-e4c3d3fd629d?auto=format&fit=crop&w=1600&q=88" alt={en ? "Jeweller working by hand in an atelier" : "珠寶工匠在工房以手工製作"} fill sizes="55vw" /><span className="editorial-caption">{en ? "Made slowly in Hong Kong" : "在香港，慢慢完成"}<small>IARA ATELIER</small></span></div></section>
