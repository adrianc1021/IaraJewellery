...
export default async function HomePage() {
  ...
  return <main id="main" className="page-shell homepage-layout" style={style}>
    ...
    <section className="home-intro reveal-item"><p className="eyebrow">IARA SIGNATURE</p><h2>為日常，收藏一束光</h2><p>Iara 源自對水與光的想像。每件作品以細膩曲線承托寶石，讓珍貴不只留在重要時刻，也自然融入每一天。</p><Link className="text-link" href="/shop">探索所有珠寶 <ArrowRight size={13} /></Link></section>
    ...
    <section className="section" id="new-arrivals"><div className="section-heading reveal-item"><div><p className="eyebrow">NEW ARRIVALS</p><h2>本季新作</h2><p>從全新輪廓到標誌系列，細選當季值得珍藏的作品。</p></div><Link className="text-link" href="/shop?sort=newest">選購所有新品 <ArrowRight size={13} /></Link></div><div className="product-grid home-new-grid">{products.map((product) => <ProductCard key={product.id} product={product} />)}</div></section>
    <section className="curation-section"><div className="curation-heading reveal-item"><p className="eyebrow">CURATED BY IARA</p><h2>從此刻，找到你的光</h2></div><div className="curation-grid">{curated.map((item) => <Link className="reveal-item" href={item.href} key={item.title}><Image src={`https://images.unsplash.com/${item.image}?auto=format&fit=crop&w=1200&q=88`} alt={item.title} fill sizes="(max-width: 680px) 100vw, 33vw" /><span><small>{item.eyebrow}</small><strong>{item.title}</strong><em>探索系列 <ArrowRight size={14} /></em></span></Link>)}</div></section>
    <section className="editorial-band" id="story"><div className="editorial-image"><Image src="https://images.unsplash.com/photo-1573408301185-9146fe634ad0?auto=format&fit=crop&w=1400&q=88" alt="珠寶工房工藝細節" fill sizes="60vw" /></div><div className="editorial-copy reveal-item"><p className="eyebrow">THE ART OF IARA</p><h2>讓工藝，成為可感受的溫度</h2><p>從寶石甄選、手工鑲嵌到最後拋光，每件作品都在香港工房細緻完成，讓光線、弧度與肌膚恰好相遇。</p><Link className="button button-light" href="/journal">閱讀工藝故事</Link></div></section>
    <section className="service-band"><div className="service-item reveal-item"><Gem size={26} /><h3>私人珠寶顧問</h3><p>由選石、尺寸到送禮建議，提供一對一服務。</p><Link className="text-link" href="/appointment">預約鑑賞 <ArrowRight size={13} /></Link></div><div className="service-item reveal-item"><MapPin size={26} /><h3>中環工作室</h3><p>親身感受寶石光澤與佩戴比例。</p><Link className="text-link" href="/appointment">查看門市 <ArrowRight size={13} /></Link></div><div className="service-item reveal-item"><ShieldCheck size={26} /><h3>安心選購</h3><p>香港免費配送，網上訂單享 14 天退換保障。</p><Link className="text-link" href="/faq">了解更多 <ArrowRight size={13} /></Link></div></section>
  </main>;
}
