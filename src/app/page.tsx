...
export default async function HomePage() {
  ...
  return <main id="main" className="page-shell homepage-layout" style={style}>
    ...
    <section className="section" id="new-arrivals"><div className="section-heading reveal-item"><div><p className="eyebrow">NEW ARRIVALS</p><h2>{en ? "New this season" : "本季新作"}</h2><p>{en ? "New silhouettes and Iara signatures, selected for this moment." : "從全新輪廓到標誌系列，細選當季值得珍藏的作品。"}</p></div><Link className="text-link" href="/shop?sort=newest">{en ? "Shop all new arrivals" : "選購所有新品"} <ArrowRight size={13} /></Link></div><div className="product-grid home-new-grid">{products.map((product) => <ProductCard key={product.id} product={product} locale={locale} />)}</div></section>
