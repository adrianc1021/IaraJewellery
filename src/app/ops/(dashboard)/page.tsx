...
export default async function OpsOverviewPage() {
  ...
  return <><OpsPageHeader eyebrow="OVERVIEW" title="營運總覽" description={`Iara 即時營運狀態 · ${formatDate(new Date())}`} />
    {(!process.env.STRIPE_SECRET_KEY || !process.env.STRIPE_WEBHOOK_SECRET || !process.env.RESEND_API_KEY || !process.env.EMAIL_FROM || !process.env.SANITY_PROJECT_ID || !process.env.SANITY_WEBHOOK_SECRET) && <p className="config-notice">部分正式整合尚未完成：請在 Render Environment 設定 Stripe Secret／Webhook、Resend Email（`RESEND_API_KEY`、`EMAIL_FROM`）及 Sanity（`SANITY_PROJECT_ID`、`SANITY_DATASET`、`SANITY_WEBHOOK_SECRET`）。在全部憑證完成前，請勿切換至正式收款。</p>}
