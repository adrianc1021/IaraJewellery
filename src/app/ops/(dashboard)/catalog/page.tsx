...
export default async function OpsCatalogPage({ searchParams }: { searchParams: Promise<{ q?: string; focus?: string }> }) {
  await requireStaff();
  ...
  return <><OpsPageHeader eyebrow="MERCHANDISING" title="商品目錄" description="新增商品、管理上下架狀態，並調整珠寶分類及品牌系列。" action={<div className="ops-header-actions"><OpsSearch value={q} placeholder="搜尋名稱、SKU、分類或系列" autoFocus={params.focus === "search"} /><span className="ops-page-count">{products.filter((product) => product.status === "ACTIVE").length} 件上架商品</span></div>} /><CatalogManager products={rows} groups={groups} /></>;
}
