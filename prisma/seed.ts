...
async function main() {
  ...
  for (const [productIndex, data] of products.entries()) {
    const product = await prisma.product.upsert({ where: { slug: data.slug }, update: {}, create: { sanityProductId: data.sanityProductId, slug: data.slug, nameZh: data.nameZh, nameEn: data.nameEn, descriptionZh: data.audience === "PET" ? "以舒適比例與圓潤邊緣設計，為珍愛的伙伴留下一道專屬光芒。" : "以細膩曲線承托寶石，讓珍貴自然融入每一天。", descriptionEn: data.audience === "PET" ? "Comfortably proportioned and softly finished for a companion's everyday signature." : "Sculpted to carry light with effortless ease, every day.", storyZh: "每件作品在香港工房經歷細緻鑲嵌、拋光與品質檢查。", category: data.category, collection: data.collection, audience: data.audience, material: data.material, gemstone: data.gemstone, badge: data.badge, featured: data.featured, imagesJson: JSON.stringify([image(data.img)]) } });
